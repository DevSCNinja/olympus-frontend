...
import { trim, secondsUntilBlock, prettifySeconds, prettyVestingPeriod, plutusBondInfo } from "../../helpers";
import { isPendingTxn, txnButtonText } from "src/actions/PendingTxns.actions";

function PlutusBondRedeem({ bond }) {
  const dispatch = useDispatch();
  const { provider, address, chainID } = useWeb3Context();

  const bondInfo = plutusBondInfo({ networkID: chainID, bond });
        ...
        <Box className="bond-data">
          <div className="data-row">
            ...
            <Typography className="price-data">
              {trim(interestDue, 4)} {bondInfo.payoutTokenTicker}
            </Typography>
          </div>
          <div className="data-row">
            <Typography>Claimable Rewards</Typography>
            <Typography className="price-data">
              {trim(pendingPayout, 4)} {bondInfo.payoutTokenTicker}
            </Typography>
          </div>
          <div className="data-row">
...
export default PlutusBondRedeem;
